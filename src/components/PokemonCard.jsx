...
function PokemonCard(props) {
    ...
    useEffect(() => {
        ...
        if (list) {
            const temp = list.find(x => (x.id == results.id))
            setData(temp)
        }
    }, [results])
    ...
    return (
        <div className="mx-4 my-4 w-80 h-30">
            <div className="text-center flex items-center justify-center bg-white rounded-md">
                <Link to={`/detail/${results.name}`} className="flex items-center">
                    <img src={results.image} className="object-cover" />
                    <h1 className="ml-4 text-lg text-green-600 font-bold">{results.name}</h1>
                </Link>
